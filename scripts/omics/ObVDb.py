...
OUTPUT_PDF = "Obese_vs_Diabetes_individual_heatmap_fixed_sample_order.pdf"
...
def main():
    ...
    title_ax.text(
        0.5,
        0.70,
        "Person-by-Person Ion-Channel / Transporter Expression Heatmap: Obese vs Diabetes",
        ha="center",
        va="center",
        fontsize=16,
        fontweight="bold"
    ...
    title_ax.text(
        0.5,
        0.20,
        "Genes hierarchically clustered by Obese + Diabetes expression and oriented from higher to lower mean centered expression; samples retained in original order",
        ha="center",
        va="center",
        fontsize=9
    )
    ...
    strip_ax.text(
        len(OBESE_SAMPLES) + (len(DIABETIC_SAMPLES) - 1) / 2,
        1.55,
        "Diabetes",
        transform=strip_ax.get_xaxis_transform(),
        ha="center",
