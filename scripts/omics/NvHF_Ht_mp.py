...
OUTPUT_PDF = "Normal_vs_HF_individual_heatmap_fixed_sample_order.pdf"
...
def main():
    ...
    title_ax.text(
        0.5,
        0.70,
        "Person-by-Person Ion-Channel / Transporter Expression Heatmap: Normal vs Heart Failure",
        ha="center",
        va="center",
        fontsize=16,
        fontweight="bold"
    ...
    title_ax.text(
        0.5,
        0.20,
        "Genes hierarchically clustered by Normal + HF expression and oriented from higher to lower mean centered expression; samples retained in original order",
        ha="center",
        va="center",
        fontsize=9
    )
    ...
    strip_ax.text(
        len(NORMAL_SAMPLES) + (len(HF_SAMPLES) - 1) / 2,
        1.55,
        "Heart Failure",
        transform=strip_ax.get_xaxis_transform(),
        ha="center",
