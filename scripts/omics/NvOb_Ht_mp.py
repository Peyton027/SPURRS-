...
OUTPUT_PDF = "Normal_vs_Obese_individual_heatmap_hierarchical.pdf"
COLOR_LIMIT = 2.0
...
def main():
    ...
    if not np.isfinite(matrix).all():
        raise ValueError(
            "All Normal and Obese sample columns must contain finite numeric values."
        )
    ...
    title_ax.text(
        0.5,
        0.20,
        "Genes hierarchically clustered by Normal + Obese expression pattern and oriented from higher to lower mean centered expression; samples clustered within cohort",
        ha="center",
        va="center",
        fontsize=9
    )
